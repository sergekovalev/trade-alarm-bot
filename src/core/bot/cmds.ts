...
export const init = (msg: any, match: any) => {
  const { is_bot: isBot, language_code: languageCode } = msg.from
...
export const setWallet = async (msg: any, match: any) => {
  ...
  const tickers = (await db.getTickers()).map((ticker: string) =>
    ticker.toLowerCase()
  )
  ...
  if (!tickers.includes(ticker)) {
    return bot.sendMessage(
      chatId,
      `Unsupported ticker «${ticker.toUpperCase()}»`
    )
  }
...
export const getPrice = async (msg: any, match: any) => {
  ...
  try {
    ...
    bot.sendMessage(
      chatId,
      `Error while getting price for ${ticker.toFixed(2)}`
    )
  }
}
